...
def require_password():
    """Show a reliable email/password login gate.

    Uses Streamlit forms so typing and pressing Enter submits the form,
    while the explicit button also works with a normal mouse click.
    """
    ...
    with mid:
        ...
        with login_tab:
            ...
            if login_clicked:
                ...
                elif user and _verify_password(
                    password, user.get("salt", ""), user.get("password_hash", "")
                ):
        ...
        with signup_tab:
            ...
            st.caption("Create an account using your email address and your own password.")

            with st.form("signup_form", clear_on_submit=False, enter_to_submit=True):
                ...
                create_clicked = st.form_submit_button(
                    "✨ Create Account & Login", type="primary", use_container_width=True
                )
            ...
            if create_clicked:
                ...
                if not _valid_email(email_key):
                    ...
                    st.error("This email is already registered. Please use Sign In.")
                else:
